...
uint32_t cTemplate_Database_Manager::LoadFile(const char* filename)
{
    ...
    try 
    {
            ...
            switch (input.peek()){
                    ...
                    switch(in_conditional){
                        ...
                        case false:
                            ...
                            try 
                            {
                                ...
                                in_conditional = true;
                                database->conditional_addresses.emplace(item.conditional_address);
                                continue;
                            }catch(const std::invalid_argument) {
                ...
                // format " 0x00 c <string> " 'address register_type value'
                    ...
                    *item.pValue = token.substr(token.find_first_not_of(" "), token.find_last_not_of(" ") );
                    ...
                    // store the item.
                    database->content.emplace(item);
            }
        }
        file.close();
        database->id_number = this->Template_counter++;
        ...
        //add new database
        Template_list.push_back(database);
    }
